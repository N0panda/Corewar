...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/02 17:40:45 by mdelory           #+#    #+#             */
/*   Updated: 2019/11/28 17:14:01 by mdelory          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "exec.h"

static void		xor_id(t_process *proc, t_vm *vm)
{
	int			ret;
...
static void		xor_dir(t_process *proc, t_vm *vm)
{
	int			ret;
...
static void		xor_reg(t_process *proc, t_vm *vm)
{
	int			ret;
	...
	{
		ret = proc->reg[vm_read(vm, proc->pc + 2, 1, REG)] ^ vm_read(vm,
				proc->pc + (vm_read(vm, proc->pc + 3, 2, 0) % IDX_MOD), 4, 0);
		proc->reg[vm_read(vm, proc->pc + 5, 1, REG)] = ret;
	}
	proc->carry = !ret;
}
